...
reverted). Script: `finish_with_fix.sh`, log `finish_with_fix5.log` in this session's
scratchpad. **Check that log's real output before assuming any outcome -- table_structural's
holdout data from the fix-4 run is now stale/invalid and will be freshly regenerated by this
run (the checkpoint logic in `run_chain` never trusts "blocked" as terminal, so this happens
automatically on re-invocation).**
