...
Status: exploratory, not confirmatory. Two hand-authored fixtures, one trial per arm each
(4 real `claude` CLI trials total) -- not part of the locked PAPER-S7 corpora, families, or
...
All 4 trials matched ground truth, manually spot-checked against the actual report text (not
...
`<w:jc w:val="left"/>` attribute, for instance) -- not a guess. Treatment's reports came from
actually calling `audit_equation_style` (one response also cross-checked with
`get_document_review`) and reporting its structured findings accurately.

## Honest conclusion

At this sample size (n=2 documents, 1 trial per arm each), **no capability gap between arms
on this task**. This is not a claim that none exists -- two documents is nowhere near enough
data to support any statistical claim, and this was never intended as one. It is a clean
signal that the task shape itself works (the harness correctly captures and grades a
diagnosis task, not just edit tasks) and that a genuine, hand-verified violation was neither
missed by either arm nor hallucinated by either arm on the compliant fixture. Scaling this to
a real sample -- more documents, more violation types (numbering gaps/duplicates, and
figure/table spacing once `audit_figure_table_spacing` stabilizes in the shared repo) -- is
