...
\node[box] (agent) at (0, 8.2) {Agent (Claude)};
\node[box] (mcp) at (0, 6.6) {Meridian Docs MCP server};
\node[box] (edit) at (0, 5.0) {Apply that family's structural edit\\\footnotesize (shared save/lock path, all six families)};
\node[decision] (needsgate) at (0, 3.0) {Family needs\\render verification?};

\node[branch] (direct) at (-3.4, 0.9)
  {\textbf{No} -- already persisted\\
   \footnotesize Bibliography, citation,\\[-1pt]
   \footnotesize section reorder};
\node[branch] (gated) at (3.4, 0.9)
  {\textbf{Yes} -- render-gate verify,\\
   \footnotesize then persist or rollback (Figure~\ref{fig:rendergate-arch})\\[-1pt]
   \footnotesize Equation, table, caption};

\node[box] (docx) at (0, -1.4) {Persisted \texttt{.docx}};

\draw[arr] (agent) -- (mcp);
\draw[arr] (mcp) -- node[lbl, right] {calls one bounded tool} (edit);
...
\draw[arr] (gated) -- (docx);
\end{tikzpicture}
\caption{Meridian Docs' overall functional architecture. Editing is a shared step for all six
families; render-gate verification is a sequential, conditional step layered on \emph{after} the
edit, only for the three structural families, detailed in Figure~\ref{fig:rendergate-arch} --
not an independent alternative path chosen from the start. This split is exactly the property
that Section~\ref{sec:render-gate} shows produces markedly different reliability behavior.}
...
Five factors limit how far these results generalize. \textbf{First}, this benchmark tests one
...
operations, and read-only structural/indexing tools including \texttt{locate\_anchor} and
\texttt{index\_document} -- are outside this benchmark's scope, and we do not claim these
results generalize to the full tool surface. Relatedly, every trial reported here receives its
target anchor pre-resolved at corpus-build time (Section~\ref{sec:methods}); the live
anchor-resolution tools' own reliability under real, uncontrolled use is a real question this
