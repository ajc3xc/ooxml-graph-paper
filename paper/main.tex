...
host conditions; we report their live-verified pass rates rather than treating them uniformly as
a capability finding -- whole-table insertion reaches a genuinely high 88.5\%, while equation
(46.2\%) and caption (7.7\%) remain far lower. We directly tested and rejected the hypothesis
that this gap reflects a difference in how expensive each family is to render-verify (a
controlled, isolated timing test found no such difference); the honest attribution is a mix of
shared-host scheduling contention and a physically unreliable storage device on our own test
host, neither of which is a property of the editing operations themselves. Separately, we
document seven
...
\emph{passed}.}
  ...
  & treatment & 46.2\% & [26.9, 65.4] & 26 & & \multirow{2}{*}{final} \\
\addlinespace
...
\bottomrule
\end{tabular}
\caption{Write-time structurally-verified families, all rows now final and live-verified.
Control never invokes the verification step (it edits raw XML directly) and is unaffected by
it. Equation's treatment $n=26$ has no paired significance test because its control arm comes
from a different, unpaired corpus (see footnote). Caption's $p<0.001$ reflects
verification-\emph{reliability} cost under host contention, not an
...
We initially suspected this gap reflected a real difference in rendering-verification cost
between task types -- e.g., that inserting a numbered caption field forces the renderer to
recompute field numbering across the whole document before it can produce output, while a bare
table does not. We tested this directly rather than let it stand as a plausible-sounding
story: a code-level review of the tools' actual OOXML output found no field-dirtying or
document-recalculation mechanism in any of the three insertion tools, and a controlled,
sequential timing test -- rendering real completed output files from all three families,
interleaved to control for host-condition drift -- found no meaningful difference (equation
8.0s, table-structural 8.6s, caption 8.9s mean, all $\ll$ the 90s timeout). We reject the
task-dependent-verification-cost hypothesis on this direct evidence. The honest attribution is
two independent, real causes neither of which is a property of the editing operations
themselves: ordinary shared-host scheduling contention (real, but evidently not sufficient on
its own -- an isolated, single-worker re-run of caption's remaining chains still failed almost
completely), and, found directly while investigating that isolation result, a physically
unreliable external drive on our test host that disconnected twice during this project (Windows
disk-layer I/O retry/error events independently confirm this, not just an inference from missing
files). We report this plainly rather than paper over an inconclusive root cause with a tidier-
sounding but empirically rejected story.
...
believe this fully resolves the question for equation and caption insertion specifically. We
directly tested and rejected the most obvious alternative explanation -- that their tools are
simply more expensive to render-verify than table insertion's (Section~\ref{sec:render-gate}) --
and traced the gap instead to shared-host contention and a physically unreliable test-host
component, neither a property of the editing operations. Caption's number did not close even
once host contention was isolated away, so a genuine remaining capability gap for caption
specifically cannot yet be ruled out either; we report that uncertainty rather than resolve it
by assumption.
...
inferred). We cannot rule out that this contributed to some fraction of equation's and
caption's still-low numbers in a way indistinguishable, from inside the harness, from ordinary
render-verification timeouts. We disclose this as a real limitation of our specific test
environment rather than a property of the tools under test, since a controlled timing test
(Section~\ref{sec:render-gate}) directly rejected the alternative explanation (task-dependent
render cost) and an isolated, contention-free re-run of caption's remaining chains still failed
almost completely -- consistent with an intermittent hardware cause neither fully explained by,
nor fully ruled out as, the reason caption in particular remains far below equation and
table-structural.
...
generic-article format; (3) generate real figures from the underlying run data rather than
tables alone; (4) expand Related Work with a fuller, independently-verified literature pass.}
