...
the true original document state across repeated cycles, a pattern we call \emph{state drift}. Three families (equation, table-structural, and caption insertion) share a
...
This benchmark's harness, evaluator, statistics code, and corpus manifests all exist in a private
git repository; none of it is publicly released as of this draft.
\pending{decide, before submission: what gets released (harness/evaluator code, statistics code,
corpus manifests -- not the licensed DocOps documents themselves), under what license, and hosted
where, then update this statement and Section~\ref{sec:defects}'s item 6 with the real link
rather than leaving both as a placeholder.}
...
research questions, the Clopper-Pearson defect/fix, the artifact-availability note, and the
...
from this generic-article format to that venue's actual template and page limit; (3) resolve
Section~\ref{sec:limitations}'s pending artifact-availability decision (what gets released,
under what license, hosted where) and update that section and defect 6's citation with the real
link.}
