...
a capability finding -- whole-table insertion reaches a genuinely high 96.2\%, while equation
...
shared-host scheduling contention (directly traced in part to an unrelated, legitimate,
CPU-and-memory-heavy background workload on our own test host), a physically unreliable storage
device on that same host, and confirmed pure network failures, none of which is a property of
the editing operations themselves. Separately, we
document eight
...
\item We disclose eight infrastructure, harness, and evaluation-code defects found during this
...
persisted change is correct} -- table-structural's 96.2\% pass rate, statistically
indistinguishable from its own control on the identical verification mechanism, shows the
mechanism itself does not inherently depress correctness once it succeeds. We therefore report
this $p$-value in full rather than omit it,
but do not count it alongside Section~\ref{sec:section-reorder}'s result as a second
capability-level finding.

\section{Eight real defects found and fixed}
\label{sec:defects}
