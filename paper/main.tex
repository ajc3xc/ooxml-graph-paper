...
\item We show that a confirmatory benchmark's own numbers are only as trustworthy as its
  harness: of ten infrastructure, harness, and evaluation-code defects found during this
  project, several silently altered an already-reported result before independent
  re-verification caught them -- each disclosed with its root cause, fix, and re-verification
...
with. Per CONSORT's own outcome-reporting guidance for paired designs \citep{schulz2010consort},
we report each arm's own marginal pass rate with its own CI alongside a paired significance test
-- both are standard, not redundant. What we do not report is a confidence interval on the paired
difference itself (only its p-value) for the three families where that difference is doing real
interpretive work (section reorder at $K=1$ original, $K=1$ combined, and $K=4$); computing it
correctly needs the underlying per-document paired outcomes, which this draft does not
recompute from raw data, so we name the gap rather than silently report only the point estimate.
A chain that never executed or never finished because of a
...
its LibreOffice/\texttt{soffice} backend runs (tried first in the fallback order described in
Section~\ref{sec:defects}; Word COM is invoked only if it fails). This does not change what is
being measured: an audit of every persisted chain result across the original host's full run
history found the Word COM backend never succeeded on its own in any recorded chain (it only
ever appears in failure records where both backends failed together). We are explicit that this
audit is observational, not a controlled ablation: it reports which backend actually succeeded
under the existing sequential fallback order on real trials, not the result of deliberately
forcing each backend alone on the same documents, which the code supports
(\texttt{check\_render\_capability}'s own \texttt{backends} parameter) but which we did not run.
We read the observational result as strong evidence that a LibreOffice-only environment is
evaluating the same effective verification path the original run already relied on almost
exclusively, but a genuine backend effect on the measured pass rates cannot be fully ruled out
without that ablation. Second, the CLI executing each trial
...
\addlinespace
Inline citation$^{\text{a}}$ & control   & 100\% & [86.8, 100]$^\dagger$ & 26 & \multirow{2}{*}{1.0} \\
...
\addlinespace
Section reorder (combined, v1+v2)$^{\text{b}}$ & control   & 83.6\% & [74.5, 92.7] & 55 & \multirow{2}{*}{0.271} \\
...
express any uncertainty there. Notes a--b: Appendix~\ref{app:notes}.}
...
\draw[arr] (c3) -- (c4);
\end{tikzpicture}
\caption{The mechanism behind Table~\ref{tab:k4}'s result. \emph{C1--C4} label the four repeated
edit-undo cycles. Treatment re-resolves its target by
...
Figure~\ref{fig:rendergate-arch} shows this flow. The verification step tries two independent
renderer backends in a fixed fallback order, advancing to the second only if the first fails
(Section~\ref{sec:defects}, defect 7, which also corrects an earlier draft's claim that this
runs as a concurrent race) -- the pass/fail decision is unaffected by which backend succeeds.
...
significance test here because its control arm comes from a different, unpaired corpus (note
c, Appendix~\ref{app:notes}) -- resolved in Table~\ref{tab:render-gate-clean}, which pairs both on the same corpus.
...
original sample, so the percentile method itself cannot express any uncertainty there. Note c:
...
\item \textbf{Sequential backend fallback could cost more time than the harness's own outer
  budget allowed -- and the fix for it never actually shipped.} The write-time verification
  step's two backends (Section~\ref{sec:render-gate})
  ...
  the harness's 600-second outer per-trial timeout. A fix racing both backends concurrently
  instead was written and committed, so discovering a double failure would cost at most the
  slower of the two (approximately 180 seconds) rather than their sum. \textbf{Correction:}
  independently re-auditing this defect while investigating a separate reviewer comment found
  that commit was never merged into the branch this benchmark's own tooling actually runs
  from -- every reported chain, including the confirmatory re-run, executed against the
  original sequential-fallback code, not the described concurrent race. We are explicit that
  this changes latency under contention, not the underlying success probability -- both
  backends are starved by the same real resource shortage regardless of whether they are raced
  or chained -- so this correction changes no reported pass/fail number: a dedicated,
  uncontended confirmatory host has no double-backend-timeout latency problem to race against
  in the first place. What it does mean is that on the original contended host, a chain whose
  both backends genuinely failed may still have been silently killed by the outer timeout
  mid-verification rather than gracefully reporting "both backends failed," exactly the failure
  mode this fix was meant to prevent.
...
Bibliography and citation insertion showing parity (Section~\ref{sec:results}) is itself likely a
correct finding, not a benchmark design flaw: both tools write already-resolved metadata into an
already-valid paragraph, a purely mechanical formatting step a capable generic agent can match. A
genuinely harder version of this task family exists, structurally analogous to section reorder's
$K=4$ finding: resolving a citation \emph{against a live external system} (\texttt{resolve\_citations}
against a local Zotero library) rather than formatting metadata already supplied in the prompt --
a real, mechanistically explainable gap a generic agent would need to independently
discover the local API's protocol and error semantics to close, not simply write correct APA
text. We did not test this; it is future work, not a claim.

A genuinely blind confirmatory re-run -- now that the tooling's own defects (Section~\ref{sec:defects})
are fixed -- is also worth naming explicitly rather than only implying: this benchmark's licensed
DocOps corpus is fully exhausted (every document already used, per Limitations factor Six), but a
separate, already-PII-screened pool of 35 organic documents from this project's own acquisition
work sits unused by the editing benchmark, and a further $\sim$12{,}000 candidate documents remain
identified but never fetched. Either is a bounded, near-term acquisition-and-screening pass, not
a from-scratch effort, and either would be closer to a true blind holdout than this paper's own
corpus now is.
...
\appendix
\section{Corpus and exclusion notes}
\label{app:notes}
Moved here from inline table footnotes, which otherwise float to the bottom of the page
disconnected from the row that triggers them.

\begin{description}
\item[Note a (Table~\ref{tab:single-edit}, Inline citation)] Treatment $n=25$: one forward call
  hit a genuine 300-second process-level subprocess timeout unrelated to the citation task
  itself, excluded from the denominator per Section~\ref{sec:methods}'s harness-flake rule
  rather than scored as a failure.
\item[Note b (Table~\ref{tab:single-edit}, Section reorder combined)] Control $n=55$, treatment
  $n=56$: one 1.7MB document's control-arm chain never completed within the harness's
  300-second timeout across six independent attempts at varying host load (a real,
  document-size-driven control-arm limitation, excluded from control's denominator only), while
  its treatment-arm chain completed and passed.
\item[Note c (Table~\ref{tab:render-gate}, Equation)] Control ($n=11$) comes from an 11-document
  development-slice corpus run once under this project's confirmatory design, not the
  26-document confirmatory corpus treatment uses; the two are not paired and no significance
  test is reported for this row. 10 of 11 resolved chains passed; the 11th resolved (did not
  block on the verification step) but failed grading on a genuine task mistake -- 11/11
  \emph{resolved} is not the same as 11/11 \emph{passed}.
\end{description}
