...
(42.3\%) and caption (7.7\%) remain far lower, a difference we attribute to task-dependent
verification cost rather than a difference in editing correctness. Separately, we document seven
...
\emph{passed}.}
  ...
  & treatment & 42.3\% & [23.1, 61.5] & 26 & & \multirow{2}{*}{final} \\
\addlinespace
...
\item \textbf{A transient network/certificate failure, not a capability finding.} We
  ...
  rather than counting them as capability failures: equation recovered from 4/26 to 11/26 and
  caption from 1/26 to 2/26 (Table~\ref{tab:render-gate}; four of equation's recoveries needed a
  second retry after an unrelated, separately transient \texttt{BadZipFile} read error on the
  same shared host reading an independently-verified-valid source file). Before this item,
  Table~\ref{tab:render-gate}'s numbers for these two families had silently included these 29 as
  ordinary capability failures -- a disclosure/reporting inconsistency corrected here and in
  that table.
...
small before the follow-up corpus is combined in. \textbf{Fourth}, the render-verification-cost
hypothesis in Section~\ref{sec:discussion} is only partially supported by the final numbers:
equation's live-verified rate (42.3\%) moved substantially closer to table-structural's (88.5\%)
once every genuinely recoverable trial was accounted for, consistent with the hypothesis, but
caption's (7.7\%) did not move nearly as far -- so either caption's verification cost is
genuinely much higher than equation's and table-structural's, or a real, still-uncharacterized
capability gap exists for caption specifically. This benchmark cannot distinguish between those
two explanations with the data collected here.
