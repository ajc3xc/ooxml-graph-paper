...
task-dependent-verification-cost hypothesis on this direct evidence. Our working attribution at
that point was two independent, real causes neither of which is a property of the editing
operations themselves: ordinary shared-host scheduling contention (real, but evidently not
sufficient on its own -- an isolated, single-worker re-run of caption's remaining chains still
failed almost completely), and, found directly while investigating that isolation result, a
physically unreliable external drive on our test host that disconnected twice during this project
(Windows disk-layer I/O retry/error events independently confirm this, not just an inference from
missing files). Rather than leave this as an inference from indirect evidence, we tested it
directly with a controlled re-run (Section~\ref{sec:methods-rerun}).
...
\paragraph{On caption's originally significant gap.} Table~\ref{tab:render-gate}'s paired test
for caption returned $p<0.001$ on the original host -- by the same statistical test as
Section~\ref{sec:section-reorder}'s headline result, that was significant. We argued at the time
that it should not be read the same way: Section~\ref{sec:section-reorder}'s finding measures
whether an agent, given a chance to attempt a task, does it correctly, while here treatment's
write-time verification step declines to persist a change at all whenever it cannot confirm
correctness under the host conditions of the moment, which is a gap in \emph{how often the arm
successfully persists a change}, not in \emph{whether a persisted change is correct}. The
confirmatory re-run settles this distinction empirically rather than leaving it as an argued
interpretation: on a dedicated host, caption's treatment reaches the identical 100\% as its
control ($p=1.0$), exactly as the verification-reliability reading predicted and the
capability-limitation reading did not.
...
disclosed engineering work (Section~\ref{sec:defects}) and, ultimately, a controlled confirmatory
re-run (Section~\ref{sec:methods-rerun}), not just a larger sample size or a more careful
argument. We did not stop at rejecting the most obvious alternative explanation -- that these
tools are simply more expensive to render-verify than table insertion's -- and then rest on an
inferred attribution to shared-host contention and a physically unreliable test-host component.
We tested that attribution directly, by re-running the identical benchmark on a dedicated,
single-tenant host, and the gap closed completely for both equation and caption
(Table~\ref{tab:render-gate-clean}). We think the general lesson generalizes past this specific
benchmark: a write-time verification step's reported reliability is only as meaningful as the
host it was measured on, and a benchmark result that looks like a capability finding is worth
testing against a change of deployment environment before it is reported as one.
